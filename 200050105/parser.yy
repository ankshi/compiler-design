      ...
      if(pl!=NULL){
		...
		i<(int) (pl->param.size());++i)
        {
		  ...
		  st->push(param->identifier,
			...
			data_type.size(),
			total_params_size + PARAM_OFFSET - cumul-4, // offset
			NULL));
    ...
    IDENTIFIER '(' ')' ';'
{
		...
		SymbTabEntry *temp = gst.search($1);
		// cout<<temp->offset;
		$$ = new funcall_astnode(new identifier_astnode($1,temp->offset,temp->size));
		if (predefined.find($1) == predefined.end())
...
equality_expression :
  ...
  | equality_expression EQ_OP relational_expression {
		...
		if (!isScalarOrPointerType(normalize(convert($1->data_type))) ||
			...
			!compatible($1->data_type,$3->data_type))
...
postfix_expression: 
  ...
  | postfix_expression '.' IDENTIFIER {
		...
		// cout<<temp->size<<"pasrer"<<endl;
		$$ = new member_astnode($1, new identifier_astnode($3,identifier_entry->offset,identifier_entry->size)); // Add in lex
  ...
  | postfix_expression PTR_OP IDENTIFIER{
		...
		$$ = new arrow_astnode($1, new identifier_astnode($3,identifier_entry->offset,identifier_entry->size)); // Add in lex
		$$->data_type = identifier_entry->type;
		$$->lvalue = true;
  }
...
primary_expression:
    IDENTIFIER {
		...
		// cout<<temp->offset;
		$$ = new identifier_astnode($1,temp->offset,temp->size);
		$$->data_type = temp->type;
		$$->lvalue = true;
}
